...
/**
 * {@link RetentionStrategy} that allows to take an agent offline for a defined time window for
 * maintenance.
 */
public class AgentMaintenanceRetentionStrategy extends RetentionStrategy<SlaveComputer> {

  private static final Logger LOGGER =
      Logger.getLogger(AgentMaintenanceRetentionStrategy.class.getName());

  private RetentionStrategy<SlaveComputer> regularRetentionStrategy;

  @DataBoundConstructor
  public AgentMaintenanceRetentionStrategy(
      RetentionStrategy<SlaveComputer> regularRetentionStrategy) {
  ...
  public void setRegularRetentionStrategy(
      RetentionStrategy<SlaveComputer> regularRetentionStrategy) {
    this.regularRetentionStrategy = regularRetentionStrategy;
  }
  ...
  public synchronized long check(final SlaveComputer c) {
    ...
    LOGGER.log(
        Level.FINER,
        "Checking for Maintenance Window for agent {0}. online = {1}, idle = {2}",
        new Object[] {c.getName(), c.isOnline(), c.isIdle()});
    if (maintenance != null) {
      LOGGER.log(
          Level.FINE,
          "Active Maintenance Window found for agent {0}: startTime = {1}, endTime = {2}",
        ...
        if (maintenance.isKeepUpWhenActive()) {
          if (!maintenance.isMaxWaitTimeFinished()) {
              ...
              Queue.withLock(
                  new Runnable() {
                    ...
                    public void run() {
                      LOGGER.log(
                          Level.INFO,
                          "Disconnecting agent {0} as it was idle when "
            ...
            if (maintenance.isAborted()) {
              LOGGER.log(
                  Level.INFO,
                  "Disconnecting agent {0} as it has finished its scheduled uptime and max "
                      + "waiting time for builds to finish is over",
                  new Object[] {c.getName()});
              c.disconnect(maintenance.getOfflineCause());
            } else {
              LOGGER.log(
                  Level.INFO,
                  "Aborting running builds on agent {0} as it has finished its scheduled uptime "
                      + "and max waiting time for builds to finish is over",
                  new Object[] {c.getName()});
          ...
          if (maintenance.isAborted()) {
            ...
            LOGGER.log(
                Level.INFO,
                "Disconnecting agent {0} as it has finished its scheduled uptime",
                new Object[] {c.getName()});
            c.disconnect(maintenance.getOfflineCause());
          } else {
            LOGGER.log(
      ...
      if (c.isOffline()) {
        ...
        if (oc instanceof MaintenanceOfflineCause) {
          ...
          if (!moc.isTakeOnline()) {
            LOGGER.log(
                Level.INFO, "Computer should not be taken online automatically: {0}", c.getName());
            return 5;
          }
        }
      }
  ...
  public static class DescriptorImpl extends Descriptor<RetentionStrategy<?>> {
     ...
     */
    public final List<Descriptor<RetentionStrategy<?>>> retentionStrategyDescriptors(
        @CheckForNull Slave it) {
      List<Descriptor<RetentionStrategy<?>>> descriptors =
          it == null

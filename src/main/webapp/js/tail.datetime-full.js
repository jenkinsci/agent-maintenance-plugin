    ...
    datetime.prototype = {
        ...
        bind: function(){
            ...
            if(typeof this._bind === "undefined"){
                ...
                this.e.addEventListener("keydown", function(event){
                    self.callback.call(self, event);
                });
        ...
        callback: function(event){
            ...
            if(event.type == "keydown"){
                ...
                if((event.keyCode || event.which) == 9){ // TAB
                    if(!this.con.stayOpen){ this.close(); }
                }
            }
        },
        ...
        fetchDate: function(date){
            ...
            if(inp ) {
            	if (inp.length > 0) {
            		date.setHours(inp[0].value || 0);
            	}
            	if (inp.length > 1) {
            		date.setMinutes(inp[1].value || 0);
            	}
            	if (inp.length > 2) {
            		date.setSeconds(inp[2].value || 0);
            	}
            }
            return date;
        },
